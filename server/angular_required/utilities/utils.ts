...
export function customFilterPredicate() {
  const customFilterFn = function (
    data: any /* Type of form data */,
    filter: string,
  ): boolean {
    ...
    for (const key in filteredValues) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(filteredValues, key) &&
        key !== "globalFilter"
      ) {
    ...
    return (
      isIndividualFilterMatchFound.every((found) => found) &&
      isGlobalFilterMatchFound
    );
  };
 ...
 * @returns boolean
 */
export function filterWithMatchMode(
  value: string,
  filter: string,
  matchMode: string,
) {
  if (matchMode === "startsWith") {
    return value
      ...
      .trim()
      .toLowerCase()
      .startsWith(filter.toString().trim().toLowerCase());
  }
  if (matchMode === "endsWith") {
    return value
      ...
      .trim()
      .toLowerCase()
      .endsWith(filter.toString().trim().toLowerCase());
  }
