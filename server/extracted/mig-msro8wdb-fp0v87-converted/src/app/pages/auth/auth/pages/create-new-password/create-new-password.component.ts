...
export class CreateNewPasswordComponent implements OnInit, OnDestroy {
  ...
  getQueryParam() {
    this.subscriptions.push(
      this._activatedRoute.fragment.subscribe((param) => {
        if (param) {
          const decryptData = this._encryptionService.decryptUsingAES256(decodeURIComponent(param));
          if (decryptData.route_details.username) {
            this.userName = decryptData.route_details.username;
          }
        }
      }),
    );
