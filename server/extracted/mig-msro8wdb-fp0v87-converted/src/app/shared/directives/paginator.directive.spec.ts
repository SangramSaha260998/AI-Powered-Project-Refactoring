...
function getNativeElementsByText(
  fixture: ComponentFixture<MatPaginatorApp>,
  type: string,
  text: string,
): HTMLElement[] {
...
function getElementsByText(paginatorElement: HTMLElement, type: string, text: string): Element[] {
  return Array.from(paginatorElement.querySelectorAll(type)).filter(
    (element) => element.textContent === text,
  );
}

function getElementByText(paginatorElement: HTMLElement, type: string, text: string): any {
  return Array.from(paginatorElement.querySelectorAll(type)).find(
    (element) => element.textContent === text,
  );
}
