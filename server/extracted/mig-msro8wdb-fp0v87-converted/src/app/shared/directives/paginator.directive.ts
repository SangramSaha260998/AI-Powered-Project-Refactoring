...
export class PaginatorDirective implements DoCheck, AfterViewInit {
  ...
  private buildPageNumbers = () => {
    ...
    this.renderer.addClass(pageRange, 'custom-paginator-container');
    ...
    const page: number = this.showTotalPages + 2;
    const pageDifference: number = totalPages - page;
    const startIndex: number = Math.max(this.currentPage - this.showTotalPages - 2, 1);
  ...
  public ngAfterViewInit(): void {
    ...
    this.initPageRange();
    const firstBtn = this.el.nativeElement.querySelector('.mat-mdc-paginator-navigation-first');
    if (firstBtn) {
      firstBtn.innerHTML = 'First';
    }
    const lastBtn = this.el.nativeElement.querySelector('.mat-mdc-paginator-navigation-last');
